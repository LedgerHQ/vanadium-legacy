...
use bitcoin::{bip32::Xpub, hashes::Hash, secp256k1::Secp256k1, sighash::SighashCache, Psbt, TapLeafHash, TapSighashType, XOnlyPublicKey};
use rand_chacha::ChaCha20Rng;
use schnorr_fun::{fun::{marker::{NonZero, Normal, Public, Zero}, KeyPair, Point, Scalar}, musig::{new_with_deterministic_nonces, AggKey, Nonce}, Message};
use vanadium_sdk::crypto::{CxCurve, EcfpPrivateKey};

use crate::{error::Result, state::MusigSession, taproot::{compute_taproot_sighash, tagged_hash, GetTapLeafHash, GetTapTreeHash, BIP0341_TAPTWEAK_TAG}, wallet::{musig::{get_musig_bip32_tweaks, MusigPartialSig, PsbtMuSig2Cosigner}, DescriptorTemplate, ExtendedPubKey, KeyPlaceholder, MySha256, SegwitVersion, WalletPolicy}, AppError};
...
impl<'a> PsbtMuSig2Cosigner for HotMuSig2Cosigner<'a> {
    fn get_participant_pubkey(&self) -> Point {
        let secp = Secp256k1::new();
        Point::from_bytes(Xpub::from_priv(&secp, &self.cosigner_xpriv).public_key.serialize()).unwrap()
    }
    ...
    fn generate_partial_signatures(&mut self, psbt: &Psbt) -> Result<Vec<MusigPartialSig>> {
        ...
        let my_keypair = KeyPair::<Normal>::new(my_privkey_scalar);
...
pub fn run_musig2_test(psbt: &mut Psbt, wallet_policy: &WalletPolicy, cosigners: &mut [&mut dyn PsbtMuSig2Cosigner]) -> Result<()>
{
    ...
    for musig_cosigner in &mut *cosigners {
        musig_cosigner.generate_public_nonces(psbt)?
    }
    ...
    let participant_keys: Vec<Point> = cosigners.into_iter()
        .map(|x| x.get_participant_pubkey())
        .collect();
...
mod tests {
    ...
    fn test_musig2_cosigner_keypath() -> Result<()> {
        ...
        let mut cosigner1 = HotMuSig2Cosigner::new(&wallet_policy, cosigner_1_xpriv)?; 
